...
train_tsv = "../../data/IJCAI2019_data/twitter2017/train.tsv"
...
class SentimentClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, s2_input_ids, s2_attention_mask):
        ...
        caption = self.gate_caption(outputs.pooler_output)
        face = self.gate_face(s2_outputs.pooler_output)
        gate_all = self.gate(caption * face)
        T = torch.sigmoid(gate_all)
        C = 1 - T
        outputs_save = T*caption + C*face
        
        outputs = self.drop(outputs_save)
        return self.out(outputs), outputs_save
